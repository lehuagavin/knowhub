...
```
并发（交替）:              并行（同时）:
Core 1: [A][B][A][B]       Core 1: [AAAAAAA]
                            Core 2: [BBBBBBB]
```

Python 的并发模型有四大支柱：**threading**、**multiprocessing**、**concurrent.futures**、**asyncio**。它们的选择取决于任务是 I/O 密集还是 CPU 密集，以及对并发规模的需求。
...
```
Thread A (持有 GIL):  [执行字节码] → [5ms 到期] → [释放 GIL] → [等待]
Thread B (等待 GIL):  [阻塞等待]  →             → [获取 GIL] → [执行字节码]
```
...
| 机制 | 说明 |
|------|------|
| **偏向引用计数（Biased Reference Counting）** | 每个对象维护两个引用计数：本地计数（创建线程无锁修改）+ 共享计数（其他线程原子修改），定期合并 |
| **Per-object locks** | `dict`、`list`、`set` 等内置类型使用细粒度的对象级锁，替代全局 GIL |
| **mimalloc** | 集成高性能内存分配器，free-threaded 构建必须启用 |
| **对象不朽化（Immortalization）** | 代码常量、驻留字符串等标记为"不朽"，永不释放，避免引用计数竞争 |

**性能影响**：

- 多线程 CPU 密集型：提速 3~4 倍（四核），部分场景 10 倍+
- 单线程：慢 5%~10%（原子操作开销）
- 内存：多用 15%~20%
...
### 4.2 进程启动方式（Start Methods）

这是 multiprocessing 最重要的内部机制之一：

| 方式 | 机制 | 优点 | 缺点 | 默认平台 |
|------|------|------|------|----------|
| `fork` | `fork()` 系统调用，复制父进程内存（COW） | 启动快，子进程继承父进程状态 | 父进程有线程时不安全（可能死锁） | Linux（3.13 前） |
...
# 其他进程通过 name 连接：SharedMemory(name=shm.name)
```

**pickle 的限制**：`pickle` 通过"全限定名引用"序列化函数和类，子进程必须能 import 到同一个模块。lambda、闭包、局部定义的函数通常无法 pickle。
...
**回调执行线程**：回调由完成任务的 worker 线程执行，不是主线程。多个回调按添加顺序执行。
...
```
事件循环（单线程）:
  [Task A 执行] → [await I/O] → [Task B 执行] → [await I/O] → [Task A 恢复] → ...
```
...
**Selector 是什么？** 事件循环底层依赖操作系统的 I/O 多路复用机制：
...
当协程 `await` 一个 socket 读取时，事件循环将该 socket 的 fd 注册到 epoll，然后去执行其他任务。当数据到达时，epoll 通知事件循环，协程被恢复。
...
| 数据共享 | 直接共享，需加锁 | pickle / 共享内存 | 同左 | 直接共享，无需锁（单线程） |
...
```
任务是 CPU 密集型？
├── 是 → Python 3.14t free-threaded 可用？
│        ├── 是 → threading（真正并行，共享内存更方便）
│        └── 否 → multiprocessing / ProcessPoolExecutor
└── 否（I/O 密集型）
     ├── 需要高并发（100+ 连接）？ → asyncio
     ├── 简单并发（几个到几十个）？ → threading / ThreadPoolExecutor
     └── 混合场景？ → asyncio + asyncio.to_thread() 处理阻塞调用
```
...
| 2026-2027 | 生态系统全面适配，multiprocessing 在部分 CPU 密集场景被 threading 替代 |
