...
```
fn example() {
    let x: i32 = 42;                    // 栈：直接写入当前帧
    let y: f64 = 3.14;                  // 栈：紧邻 x 之后
    let v: Vec<i32> = vec![1, 2, 3];    // 栈上 24 字节（ptr+len+cap），堆上 12 字节数据
    let s: String = String::from("Hi"); // 栈上 24 字节（ptr+len+cap），堆上 2 字节数据
}

              栈帧（编译时布局确定）                堆（运行时动态分配）
              ┌─────────────────────┐
    SP ────→  │ s: ptr ─────────────────────────→ [ 'H', 'i' ]
              │    len=2, cap=2     │
              │ v: ptr ─────────────────────────→ [ 1, 2, 3 ]
  ...
  ↑ 向低地址增长                          分散在可用内存中，由分配器管理
  移动栈指针即完成分配（~1 ns）            分配器查找空闲块（~25-100 ns）
  函数返回时整帧回收                       每个分配独立回收（drop 时释放）
  主线程 ~8 MB / 子线程 ~2 MB              受限于可用内存，几乎无上限
  编译时必须知道每个变量的大小              大小可在运行时确定
```
...
```
C 语言：   malloc → 使用 → free（容易忘记，或 double-free）
Java：     new → 使用 → GC 在"某个时候"回收（无法控制时机）
Rust RAII：创建 → 使用 → 离开作用域时自动 drop（零开销，确定性释放）
```
...
```
&Box<String> → Box.deref() → &String → String.deref() → &str ✓
```
...
**瘦指针（Thin Pointer）：** 一个 `usize`（8 字节）
...
```
切片引用 &[T]：              trait 对象 &dyn Trait：
┌──────────┐                ┌──────────┐
│ data_ptr │ → 第一个元素    │ data_ptr │ → 具体类型实例
│ length   │ → 元素个数      │ vtable   │ → 虚函数表
└──────────┘                └──────────┘
```
